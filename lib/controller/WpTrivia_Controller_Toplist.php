...
class WpTrivia_Controller_Toplist extends WpTrivia_Controller_Controller
{
    ...
    private function showAdminToplist($quizId)
    {
        ...
        $view = new WpTrivia_View_AdminToplist();
    ...
    public function getAddToplist(WpTrivia_Model_Quiz $quiz)
    {
        ...
        if ($quiz->isToplistDataCaptcha() && $userId == 0) {
            $captcha = WpTrivia_Helper_Captcha::getInstance();
    ...
    private function handleAddInToplist(WpTrivia_Model_Quiz $quiz)
    {
        ...
        $quizMapper = new WpTrivia_Model_QuizMapper();
        $toplistMapper = new WpTrivia_Model_ToplistMapper();
        ...
        if ($userId > 0) {
            if ($toplistMapper->countUser($quizId, $userId, $clearTime)) {
                return array('text' => __('You can not enter again.', 'wp-trivia'), 'clear' => true);
            }
            ...
            if (empty($name) || empty($email) || filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
                return array('text' => __('No name or e-mail entered.', 'wp-trivia'), 'clear' => false);
            }

            if (strlen($name) > 15) {
                return array('text' => __('Your name can not exceed 15 characters.', 'wp-trivia'), 'clear' => false);
            }
            ...
            if ($quiz->isToplistDataCaptcha()) {
                ...
                if ($captcha->isSupported()) {
                    if (!$captcha->check($prefix, $captchaAnswer)) {
                        return array('text' => __('You entered wrong captcha code.', 'wp-trivia'), 'clear' => false);
                    }
                }
            }
        }

        $toplist = new WpTrivia_Model_Toplist();
    ...
    {
        switch ($type) {
            case WpTrivia_Model_Quiz::QUIZ_TOPLIST_TYPE_ALL:
                return true;
            case WpTrivia_Model_Quiz::QUIZ_TOPLIST_TYPE_ONLY_ANONYM:
                return $userId == 0;
            case WpTrivia_Model_Quiz::QUIZ_TOPLIST_TYPE_ONLY_USER:
                return $userId > 0;
        }
    ...
    public static function ajaxAddInToplist($data)
    {
        ...
        $ctn = new WpTrivia_Controller_Toplist();
        ...
        if ($r === true) {
            $r = array('text' => __('You signed up successfully.', 'wp-trivia'), 'clear' => true);
        }
