...
class WpTrivia_Controller_Statistics extends WpTrivia_Controller_Controller
{
    ...
    private function showNew($quizId)
    {
        ...
        $view = new WpTrivia_View_StatisticsNew();
    ...
    public function save($quiz = null)
    {
        ...
        if ($quiz->getStatisticsIpLock() > 0) {
            ...
            $lockMapper->deleteOldLock($lockTime, $quiz->getId(), time(), WpTrivia_Model_Lock::TYPE_STATISTIC);

            if ($lockMapper->isLock($quizId, $lockIp, $userId, WpTrivia_Model_Lock::TYPE_STATISTIC)) {
                return false;
            }
            ...
            $lock->setQuizId($quizId)
                ->setLockIp($lockIp)
                ->setUserId($userId)
                ->setLockType(WpTrivia_Model_Lock::TYPE_STATISTIC)
                ->setLockDate(time());

            $lockMapper->insert($lock);
        }

        $statisticRefModel = new WpTrivia_Model_StatisticRefModel();
    ...
    private function makeFormData($quiz, $data)
    {
        ...
        foreach ($forms as $form) {
            if ($form->getType() != WpTrivia_Model_Form::FORM_TYPE_DATE) {
                ...
                if (!WpTrivia_Helper_Form::valid($form, $str)) {
                    return null;
                }
                ...
                $dateStr = WpTrivia_Helper_Form::validData($form, $date);
    ...
    private function makeDataList($quizId, $array)
    {
        ...
        foreach ($array as $k => $v) {
            $s = new WpTrivia_Model_Statistic();
            $s->setQuestionId($k);
    ...
    public static function ajaxLoadHistory($data)
    {
        ...
        foreach ($statisticModel as $model) {
            ...
            if (!$model->getUserId()) {
                ...
                if ($model->getUserName() == '') {
                    $model->setUserName(__('Deleted user', 'wp-trivia'));
                }
            }
            ...
            foreach ($forms as $form) {
                ...
                if ($form->isShowInStatistic()) {
                    $formOverview[] = $formData != null && isset($formData[$form->getFormId()])
                        ? WpTrivia_Helper_Form::formToString($form, $formData[$form->getFormId()])
                        : '----';
                }
            }
        ...
        $view = new WpTrivia_View_StatisticsAjax();
    ...
    public static function ajaxLoadStatisticUser($data)
    {
        ...
        $statisticRefMapper = new WpTrivia_Model_StatisticRefMapper();
        $statisticUserMapper = new WpTrivia_Model_StatisticUserMapper();
        $formMapper = new WpTrivia_Model_FormMapper();
        ...
        $view->userName = __('Anonymous', 'wp-trivia');
        ...
        if ($view->statisticModel->getUserId()) {
            ...
            if ($userInfo !== false) {
                ...
                $view->userName = __('Deleted user', 'wp-trivia');
            }
        }
