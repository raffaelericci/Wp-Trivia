<?php

/**
 * @property array users
 * @property WpTrivia_Model_Quiz quiz
 */
class WpTrivia_View_StatisticsNew extends WpTrivia_View_View
{

    public function show()
    {
        ?>

        <style>
            .wpTrivia_blueBox {
                padding: 20px;
                background-color: rgb(223, 238, 255);
                border: 1px dotted;
                margin-top: 10px;
            }

            .wpTrivia_modal_backdrop {
                background: #000;
                opacity: 0.7;
                top: 0;
                bottom: 0;
                right: 0;
                left: 0;
                position: fixed;
                z-index: 159900;
            }

            .wpTrivia_modal_window {
                position: fixed;
                background: #FFF;
                top: 40px;
                bottom: 40px;
                left: 40px;
                right: 40px;
                z-index: 160000;
            }

            .wpTrivia_actions {
                display: none;
                padding: 2px 0 0;
            }

            .mobile .wpTrivia_actions {
                display: block;
            }

            tr:hover .wpTrivia_actions {
                display: block;
            }
        </style>

        <div class="wrap wpTrivia_statisticsNew">
            <input type="hidden" id="quizId" value="<?php echo $this->quiz->getId(); ?>" name="quizId">

            <h2><?php printf(__('Quiz: %s - Statistics', 'wp-trivia'), $this->quiz->getName()); ?></h2>

            <p><a class="button-secondary" href="admin.php?page=wpTrivia"><?php _e('back to overview',
                        'wp-trivia'); ?></a></p>

            <?php if (!$this->quiz->isStatisticsOn()) { ?>
                <p style="padding: 30px; background: #F7E4E4; border: 1px dotted; width: 300px;">
                    <span style="font-weight: bold; padding-right: 10px;"><?php _e('Stats not enabled',
                            'wp-trivia'); ?></span>
                    <a class="button-secondary"
                       href="admin.php?page=wpTrivia&action=addEdit&quizId=<?php echo $this->quiz->getId(); ?>"><?php _e('Activate statistics',
                            'wp-trivia'); ?></a>
                </p>
                <?php return;
            } ?>

            <div style="padding: 10px 0px;" class="wpTrivia_tab_wrapper">
                <a class="button-primary" href="#" data-tab="#wpTrivia_tabHistory"><?php _e('History',
                        'wp-trivia'); ?></a>
                <a class="button-secondary" href="#" data-tab="#wpTrivia_tabOverview"><?php _e('Overview',
                        'wp-trivia'); ?></a>
            </div>

            <div id="wpTrivia_loadData" class="wpTrivia_blueBox" style="background-color: #F8F5A8; display: none;">
                <img alt="load"
                     src="data:image/gif;base64,">
                <?php _e('Loading', 'wp-trivia'); ?>
            </div>

            <div id="wpTrivia_content" style="display: block;">
                <?php $this->showHistory(); ?>
                <?php $this->showTabOverview(); ?>
            </div>

            <?php $this->showModalWindow(); ?>

        </div>

        <?php
    }

    private function showHistory()
    {
        ?>
        <div id="wpTrivia_tabHistory" class="wpTrivia_tabContent" style="display: block;">

            <div id="poststuff">
                <div class="postbox">
                    <h3 class="hndle"><?php _e('Filter', 'wp-trivia'); ?></h3>

                    <div class="inside">
                        <ul>
                            <li>
                                <label>
                                    <?php _e('Which users should be displayed:', 'wp-trivia'); ?>
                                    <select id="wpTrivia_historyUser">
                                        <optgroup label="<?php _e('special filter', 'wp-trivia'); ?>">
                                            <option value="-1" selected="selected"><?php _e('all users',
                                                    'wp-trivia'); ?></option>
                                            <option value="-2"><?php _e('only registered users',
                                                    'wp-trivia'); ?></option>
                                            <option value="-3"><?php _e('only anonymous users',
                                                    'wp-trivia'); ?></option>
                                        </optgroup>

                                        <optgroup label="<?php _e('User', 'wp-trivia'); ?>">
                                            <?php foreach ($this->users as $user) {
                                                if ($user->ID == 0) {
                                                    continue;
                                                }

                                                echo '<option value="', $user->ID, '">', $user->user_login, ' (', $user->display_name, ')</option>';
                                            } ?>
                                        </optgroup>
                                    </select>
                                </label>
                            </li>
                            <li>
                                <label>
                                    <?php _e('How many entries should be shown on one page:', 'wp-trivia'); ?>
                                    <select id="wpTrivia_historyPageLimit">
                                        <option>1</option>
                                        <option selected="selected">10</option>
                                        <option>50</option>
                                        <option>100</option>
                                        <option>500</option>
                                        <option>1000</option>
                                    </select>
                                </label>
                            </li>
                            <li>
                                <?php
                                $dateVon = '<input type="text" id="datepickerFrom">';
                                $dateBis = '<input type="text" id="datepickerTo">';

                                printf(__('Search to date limit from %s to %s', 'wp-trivia'), $dateVon, $dateBis);
                                ?>
                            </li>
                            <li>
                                <input type="button" value="<?php _e('Filter', 'wp-trivia'); ?>"
                                       class="button-secondary" id="filter">
                            </li>
                        </ul>
                    </div>
                </div>
            </div>

            <div id="wpTrivia_loadDataHistory" class="wpTrivia_blueBox"
                 style="background-color: #F8F5A8; display: none;">
                <img alt="load"
                     src="data:image/gif;base64,">
                <?php _e('Loading', 'wp-trivia'); ?>
            </div>

            <div id="wpTrivia_historyLoadContext"></div>

            <div style="margin-top: 10px;">

                <div style="float: left;" id="historyNavigation">
                    <input style="font-weight: bold;" class="button-secondary navigationLeft" value="&lt;"
                           type="button">
                    <select class="navigationCurrentPage">
                        <option value="1">1</option>
                    </select>
                    <input style="font-weight: bold;" class="button-secondary navigationRight" value="&gt;"
                           type="button">
                </div>

                <div style="float: right;">
                    <a class="button-secondary wpTrivia_update" href="#"><?php _e('Refresh', 'wp-trivia'); ?></a>
                    <?php if (current_user_can('wpTrivia_reset_statistics')) { ?>
                        <a class="button-secondary wpTrivia_resetComplete" href="#"><?php _e('Reset entire statistic',
                                'wp-trivia'); ?></a>
                    <?php } ?>
                </div>

                <div style="clear: both;"></div>
            </div>

        </div>
        <?php
    }

    private function showModalWindow()
    {
        ?>

        <div id="wpTrivia_user_overlay" style="display: none;">
            <div class="wpTrivia_modal_window" style="padding: 20px; overflow: scroll;">
                <input type="button" value="<?php _e('Close'); ?>" class="button-primary"
                       style=" position: fixed; top: 48px; right: 59px; z-index: 160001;" id="wpTrivia_overlay_close">

                <div id="wpTrivia_user_content" style="margin-top: 20px;"></div>

                <div id="wpTrivia_loadUserData" class="wpTrivia_blueBox"
                     style="background-color: #F8F5A8; display: none; margin: 50px;">
                    <img alt="load"
                         src="data:image/gif;base64,">
                    <?php _e('Loading', 'wp-trivia'); ?>
                </div>
            </div>
            <div class="wpTrivia_modal_backdrop"></div>
        </div>

        <?php
    }

    private function showTabOverview()
    {
        ?>
        <div id="wpTrivia_tabOverview" class="wpTrivia_tabContent" style="display: none;">
            <div id="poststuff">
                <div class="postbox">
                    <h3 class="hndle"><?php _e('Filter', 'wp-trivia'); ?></h3>

                    <div class="inside">
                        <ul>
                            <li>
                                <label>
                                    <?php _e('Show only users, who solved the quiz:', 'wp-trivia'); ?>
                                    <input type="checkbox" value="1" id="wpTrivia_overviewOnlyCompleted">
                                </label>
                            </li>
                            <li>
                                <label>
                                    <?php _e('How many entries should be shown on one page:', 'wp-trivia'); ?>
                                    <select id="wpTrivia_overviewPageLimit">
                                        <option>1</option>
                                        <option>4</option>
                                        <option selected="selected">50</option>
                                        <option>100</option>
                                        <option>500</option>
                                        <option>1000</option>
                                    </select>
                                </label>
                            </li>
                            <li>
                                <input type="button" value="<?php _e('Filter', 'wp-trivia'); ?>"
                                       class="button-secondary" id="overviewFilter">
                            </li>
                        </ul>
                    </div>
                </div>
            </div>

            <div id="wpTrivia_loadDataOverview" class="wpTrivia_blueBox"
                 style="background-color: #F8F5A8; display: none;">
                <img alt="load"
                     src="data:image/gif;base64,">
                <?php _e('Loading', 'wp-trivia'); ?>
            </div>

            <div id="wpTrivia_overviewLoadContext"></div>

            <div style="margin-top: 10px;">

                <div style="float: left;" id="overviewNavigation">
                    <input style="font-weight: bold;" class="button-secondary navigationLeft" value="&lt;"
                           type="button">
                    <select class="navigationCurrentPage">
                        <option value="1">1</option>
                    </select>
                    <input style="font-weight: bold;" class="button-secondary navigationRight" value="&gt;"
                           type="button">
                </div>

                <div style="float: right;">
                    <a class="button-secondary wpTrivia_update" href="#"><?php _e('Refresh', 'wp-trivia'); ?></a>
                    <?php if (current_user_can('wpTrivia_reset_statistics')) { ?>
                        <a class="button-secondary wpTrivia_resetComplete" href="#"><?php _e('Reset entire statistic',
                                'wp-trivia'); ?></a>
                    <?php } ?>
                </div>

                <div style="clear: both;"></div>
            </div>

        </div>
        <?php
    }
}
