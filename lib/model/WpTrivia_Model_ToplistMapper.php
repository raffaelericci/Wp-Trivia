...
class WpTrivia_Model_ToplistMapper extends WpTrivia_Model_Mapper
{
    ...
    public function save(WpTrivia_Model_Toplist $toplist)
    {
    ...
    public function fetch($quizId, $limit, $sort, $start = 0)
    {
        ...
        switch ($sort) {
            case WpTrivia_Model_Quiz::QUIZ_TOPLIST_SORT_BEST:
                $s = 'ORDER BY result DESC';
                break;
            case WpTrivia_Model_Quiz::QUIZ_TOPLIST_SORT_NEW:
                $s = 'ORDER BY date DESC';
                break;
            case WpTrivia_Model_Quiz::QUIZ_TOPLIST_SORT_OLD:
            default:
                $s = 'ORDER BY date ASC';
                break;
        }
        ...
        foreach ($results as $row) {
            $r[] = new WpTrivia_Model_Toplist($row);
        }
