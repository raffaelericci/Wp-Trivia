...
class WpTrivia_Model_CategoryMapper extends WpTrivia_Model_Mapper
{

    /**
     * @param string $type
     *
     * @return WpTrivia_Model_Category[]
     */
    public function fetchAll($type = WpTrivia_Model_Category::CATEGORY_TYPE_QUESTION)
    {
        $type = $type == WpTrivia_Model_Category::CATEGORY_TYPE_QUESTION ? $type : WpTrivia_Model_Category::CATEGORY_TYPE_QUIZ;
        ...
        foreach ($results as $row) {
            $r[] = new WpTrivia_Model_Category($row);
        }
    ...
    public function fetchByQuiz($quizId)
    {
        ...
        $results = $this->_wpdb->get_results($this->_wpdb->prepare('
			...
			ORDER BY
       			c.category_name
		', WpTrivia_Model_Category::CATEGORY_TYPE_QUESTION, $quizId), ARRAY_A);
    ...
    public function save(WpTrivia_Model_Category $category)
    {
    ...
    public function getCategoryArrayForImport($type = WpTrivia_Model_Category::CATEGORY_TYPE_QUESTION)
    {
        $type = $type == WpTrivia_Model_Category::CATEGORY_TYPE_QUESTION ? $type : WpTrivia_Model_Category::CATEGORY_TYPE_QUIZ;
