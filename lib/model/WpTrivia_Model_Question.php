...
class WpTrivia_Model_Question extends WpTrivia_Model_Model
{
    ...
    public function getAnswerData($serialize = false)
    {
        ...
        if (is_array($this->_answerData) || $this->_answerData instanceof WpTrivia_Model_AnswerTypes) {
            if ($serialize) {
                return @serialize($this->_answerData);
            }
        } else {
            if (!$serialize) {
                if (WpTrivia_Helper_Until::saveUnserialize($this->_answerData, $into) === false) {
                    return null;
                }
